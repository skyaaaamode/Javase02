...
class Graphgene {
    ...
    public static void bfs(GNode head){
        if(head==null){
            return;
        }
        Queue<GNode> gNodes = new LinkedList<GNode>();
        Set<GNode> nodes=new HashSet<GNode>();
        gNodes.add(head);
        nodes.add(head);
        while (!gNodes.isEmpty()){
            GNode poll = gNodes.poll();
    ...
    public static void dfs(GNode head){
        if (head==null){
            return;
        }
        Stack<GNode> gNodes = new Stack<>();
        HashSet<GNode> gNodes1 = new HashSet<>();
        gNodes.add(head);
        gNodes1.add(head);
        while (!gNodes.isEmpty())
        {
            GNode pop = gNodes.pop();
            for (GNode next:pop.nexts){
                if(!gNodes1.contains(next)){
                    gNodes1.add(pop);
                    gNodes.push(pop);
                    gNodes.push(next);
